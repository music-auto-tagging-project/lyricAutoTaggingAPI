...
def tag_extraction():
    ...
    lyric = data['musicLyric']
    sim_thresh = data.get("simThresh",0.12)
    result = auto_tag_model.get_keyword(lyric,sim_thresh)
    return jsonify(result)

if __name__ == "__main__":
    args = parse_args()

    auto_tag_model = LyricAutoTagModel(model_name=args.model_name, top_n=args.top_n)
